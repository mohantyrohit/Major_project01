...
// Get all conversations for a user (institute or student)
router.get('/conversations/:userId', verifyToken, async (req, res) => {
  const { userId } = req.params;
  const { userType } = req.query; // 'institute' or 'student'
  ...
  try {
    ...
    // Format the response to include participant information
    const conversations = await Promise.all(chats.map(async (chat) => {
      let participantId, participantName;
      
      if (userType === 'institute') {
        participantId = chat.studentId;
        participantName = chat.userName;
      } else {
        participantId = chat.instituteId;
        participantName = chat.collegeName;
      }
      ...
      return {
        ...
        participantName,
        lastMessage,
        updatedAt: chat.lastUpdated || chat.updatedAt || new Date(),
...
router.get('/:chatId', verifyToken, async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({
      success: false,
      message: 'Server error while retrieving chat'
    });
  }
});
