...
function EventsPage() {
  ...
  // Display a Cloudinary image URL
  const renderImageUrl = (cloudinaryPath) => {
    // Check if the URL is already a Cloudinary URL
    if (cloudinaryPath && cloudinaryPath.includes("cloudinary.com")) {
      return cloudinaryPath;
    }
    
    // If it's an old server path, maintain backward compatibility
    if (cloudinaryPath && !cloudinaryPath.startsWith("http")) {
      return `https://major-project01-1ukh.onrender.com/${cloudinaryPath.replace(/^\/?/, "")}`;
    }
    
    // Return the path as is if it's already a complete URL
    return cloudinaryPath;
  };
          ...
          {events.map((event, idx) => (
            <div key={idx} className="event-card">
              ...
              {event.organizerPhoto && (
                <div className="organizer-image-container">
                  <img
                    src={renderImageUrl(event.organizerPhoto)}
                    alt={event.organizerName}
                    className="organizer-image"
                  />
                </div>
